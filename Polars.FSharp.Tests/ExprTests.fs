...
type ``String Logic Tests`` () =
    ...
    member _.``Expr: String Cleaning & Parsing (Strip, Anchor, Date)`` () =
        ...
        // Strip
        Assert.Equal("abc", res.["strip_default"].GetValue<string>(0))
        Assert.Equal("abc  ", res.["lstrip"].GetValue<string>(0))
        Assert.Equal("  abc", res.["rstrip"].GetValue<string>(0))

        // Custom Strip
        Assert.Equal("key", res.["strip_custom"].GetValue<string>(3)) // __key__ -> key

        // Prefix / Suffix
        Assert.Equal("pl.rs", res.["strip_prefix"].GetValue<string>(1))
        Assert.Equal("data", res.["strip_suffix"].GetValue<string>(2)) // data.csv -> data

        // Anchors (Boolean)
        Assert.Equal(Some true, res.["is_url"].GetValueOption<bool>(1)) // https://...
        Assert.Equal(Some false, res.["is_url"].GetValueOption<bool>(0))
        Assert.Equal(Some true, res.["is_csv"].GetValueOption<bool>(  2)) // ...csv

        // ToDate
        // Row 4: "20250101"
        let d1 = res.["parsed_date"].GetValue<DateOnly>(4)
        ...
        // Strip + ToDate)
        // Row 5: "  2025-12-31  "
        let d2 = res.["chain_date"].GetValue<DateOnly>(5)
    ...
    member _.``Math Ops (BMI Calculation with Pow)`` () =
        ...
        let bobBmi = res.["bmi"].GetValue<float>(1)
        Assert.True(bobBmi > 24.69 && bobBmi < 24.70)

        let aliceSqrt = res.["sqrt_h"].GetValue<float>(0)
    ...
    member _.``Temporal Ops (Components, Format, Cast)`` () =
        ...
        Assert.Equal(2023L, res.["y"].GetValue<int64>(0))
        Assert.Equal(12L, res.["m"].GetValue<int64>(0))
        Assert.Equal(25L, res.["d"].GetValue<int64>(0))

        Assert.Equal(15L, res.["h"].GetValue<int64>(0))

        Assert.Equal(1L, res.["w_day"].GetValue<int64>(0))

        Assert.Equal("2023/12/25", res.["fmt_custom"].GetValue<string>(0))

        Assert.Equal(DateOnly(2023,12,25), res.["date_only"].GetValue<DateOnly>(0))

        // --- Row 1: 2024-01-01 00:00:00 ---
        Assert.Equal(2024L, res.["y"].GetValue<int64>(1))
        Assert.Equal(1L, res.["m"].GetValue<int64>(1))
        Assert.Equal(0L, res.["h"].GetValue<int64>(1))
    ...
    member _.``Cast Ops: Int to Float, String to Int`` () =
        ...
        // "100" -> 100
        let v1 = res.["str_to_int"].GetValue<int64>(0)
        Assert.Equal(100L, v1)

        // 1000 -> 1000.0
        let v2 = res.["int_to_float"].GetValue<float>(0)
    ...
    member _.``Control Flow: IfElse (When/Then/Otherwise)`` () =
        ...
        // Alice (95) -> A
        Assert.Equal("A", res.["grade"].GetValue<string>(0))
        // Bob (70) -> Pass
        Assert.Equal("Pass", res.["grade"].GetValue<string>(1))
        // Charlie (50) -> Fail
        Assert.Equal("Fail", res.["grade"].GetValue<string>(2))
    ...
    member _.``String Regex: Replace and Extract`` () =
        ...
        // "User: 12345" -> "User: #"
        Assert.Equal("User: #", res.["masked"].GetValue<string>(0))

        // "User: 12345" -> "12345"
        Assert.Equal("12345", res.["extracted_id"].GetValue<string>(0))
        Assert.Equal("999", res.["extracted_id"].GetValue<string>(1))
    [<Fact>]
