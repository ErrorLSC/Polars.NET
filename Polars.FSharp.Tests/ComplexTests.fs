...
type ``Complex Query Tests`` () =
    ...
    member _.``Join execution (Eager)`` () =
        ...
        let res =
            uDf
            |> DataFrame.join sDf [pl.col "id"] [pl.col "uid"] JoinType.Left
    ...
    member _.``Lazy API Chain (Filter -> Collect)`` () =
        ...
        let df =
            lf
            |> LazyFrame.filter (pl.col "a" .> pl.lit 1)
            |> LazyFrame.collect
            |> DataFrame.head 1
    ...
    member _.``GroupBy Queries With Having`` () =
        ...
        use res =
            ...
            |> DataFrame.groupBy
                ...
                |> pl.alias "decade" ]
            |> DataFrame.having (pl.len() .> pl.lit 1)
            |> DataFrame.agg [ pl.len() |> pl.alias "cnt" ]
            |> DataFrame.sortAscending [pl.col "decade"]
    ...
    member _.``List Ops: Cols, Explode, Join and Read`` () =
        ...
        let exploded =
            res
            |> DataFrame.select [ pl.col "my_name"; pl.col "my_tag_list" ]
    ...
    member _.``Lazy Reshaping: Concat All Types`` () =
        ...
        // Horizontal: [a, b]
        let dfHorz =
            LazyFrame.concatHorizontal [lf1; lf2]
            |> LazyFrame.collect
        ...
        // Vertical: [a] (rows=2)
        let dfVert =
            LazyFrame.concatVertical [lf1; lf3]
            |> LazyFrame.collect
        ...
        // lf2 (a=null, b=2)
        let dfDiag =
            LazyFrame.concatDiagonal [lf1; lf2]
            |> LazyFrame.collect
    ...
    member _.``Rolling Window (Moving Average)`` () =
        ...
        let res =
            lf
            |> LazyFrame.sortAscending [pl.col "date"]
            |> LazyFrame.withColumn (
                // 1.1: 10
                // 1.2: (10+20)/2 = 15
                // 1.3: (20+30)/2 = 25
    ...
    member _.``Time Series: Dynamic Rolling Window`` () =
        ...
        let res =
            ...
            |> LazyFrame.sortAscending [pl.col "time"]
            |> LazyFrame.withColumn (
    ...
    member _.``Test_ETL_Stream_EndToEnd: DataTable -> Polars -> DataTable`` () =
        ...
        let pipeline =
            lf
            |> LazyFrame.filter(pl.col "Region".== pl.lit "US")
            |> LazyFrame.withColumn((pl.col "Amount" * pl.lit 1.08).Alias "TaxedAmount")
            |> LazyFrame.select([
                pl.col "OrderId"
                pl.col "TaxedAmount"
                pl.col "OrderDate"
    ...
    member _.``LazyFrame: Unnest Struct`` () =
        ...
        let res =
            lf
            |> LazyFrame.unnest (pl.col "MyStruct")
            |> LazyFrame.collect
    ...
    member _.``DataFrame: Merge with selector-based batch setters`` () =
        ...
        let result =
            ...
            |> Merge.printPlan
            |> Merge.execute
            |> LazyFrame.sortAscending [pl.col "Id"]
            |> LazyFrame.collect
