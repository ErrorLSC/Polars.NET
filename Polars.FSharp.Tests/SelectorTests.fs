...
type SelectorTests() =
    ...
    member _.``Integration: GroupBy, Explode with Selectors`` () =
        ...
        let dfAgg =
            df
            |> DataFrame.groupBy [pl.col "Region"] 
            |> DataFrame.agg [pl.cs.numeric().ToExpr().Sum()]
            |> DataFrame.sortAscending [pl.col "Region"] 
    ...
    member _.``Selector: Advanced Regex Patterns and Generic Types`` () =
        ...
        let dfAlpha = df |> DataFrame.select [ pl.cs.alpha true false ]
        Assert.Equal(1L, dfAlpha.Width)
        Assert.Contains("pureAlpha", dfAlpha.Columns)

        // A-2. alpha (Unicode):  CJK included 
        let dfAlphaUnicode = df |> DataFrame.select [ pl.cs.alpha false false ]
        Assert.Equal(4L, dfAlphaUnicode.Width) // pureAlpha, 原神启动, 日文カタカナ, 한글

        // alphanumeric: 
        let dfAlphaNum = df |> DataFrame.select [ pl.cs.alphanumeric true false ]
        ...
        // cjk
        let dfCjk = df |> DataFrame.select [pl.cs.cjkCols { 
        ...
        // cjkAlphanumeric
        let dfCjkMix = df |> DataFrame.select [ 
            pl.cs.cjkCols { 
                Chinese = true; Japanese = true; Korean = true
        ...
        // IgnoreSpaces 
        let dfSpace = df |> DataFrame.select [ pl.cs.alpha true true ]
