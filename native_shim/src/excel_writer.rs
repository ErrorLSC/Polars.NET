...
pub fn write_excel_native(
        ...
        match series.dtype() {
            ...
            | DataType::UInt32 => {
                ...
                for (row_offset, opt_val) in ca_i64.iter().enumerate() {
                    if let Some(v) = opt_val {
                        worksheet
                            .write_number((row_offset + 1) as u32, col_u16, v as f64)
                            .map_err(|e| PolarsError::ComputeError(e.to_string().into()))?;
                    }
                }
            }
            ...
            | DataType::Decimal(_, _) => {
                ...
                for (row_offset, opt_val) in ca.iter().enumerate() {
                    if let Some(v) = opt_val {
                        worksheet
                            .write_string((row_offset + 1) as u32, col_u16, v)
                            .map_err(|e| PolarsError::ComputeError(e.to_string().into()))?;
                    }
                }
            }
            ...
            DataType::Float32 | DataType::Float64 => {
                ...
                for (row_offset, opt_val) in ca_f64.iter().enumerate() {
                    if let Some(v) = opt_val {
                        worksheet
                            .write_number((row_offset + 1) as u32, col_u16, v)
                            .map_err(|e| PolarsError::ComputeError(e.to_string().into()))?;
                    }
                }
            }
            ...
            DataType::Boolean => {
                ...
                for (row_offset, opt_val) in ca.iter().enumerate() {
                    if let Some(v) = opt_val {
                        worksheet
                            .write_boolean((row_offset + 1) as u32, col_u16, v)
                            .map_err(|e| PolarsError::ComputeError(e.to_string().into()))?;
                    }
                }
            }
            ...
            DataType::Datetime(_, _) => {
                ...
                for (row_offset, opt_val) in ca.phys.iter().enumerate() {
                    if let Some(micros) = opt_val {
                        if let Some(dt) =
                            DateTime::from_timestamp_micros(micros).map(|d| d.naive_utc())
                        {
                            worksheet
                                .write_datetime_with_format(
                                    (row_offset + 1) as u32,
    ...
    workbook
        .save(path)
        .map_err(|e| PolarsError::ComputeError(format!("Failed to save excel: {}", e).into()))?;
...
pub extern "C" fn pl_write_excel(
    ...
    ffi_try_void!({
        ...
        let sheet_name = if !sheet_name_ptr.is_null() {
            Some(ptr_to_str(sheet_name_ptr).map_err(|e| {
                PolarsError::ComputeError(format!("Invalid sheet name: {}", e).into())
            })?)
        } else {
            None
        };
        ...
        let date_fmt = if !date_fmt_ptr.is_null() {
            Some(ptr_to_str(date_fmt_ptr).map_err(|e| {
                PolarsError::ComputeError(format!("Invalid date format: {}", e).into())
            })?)
        } else {
            None
        };
        ...
        let datetime_fmt = if !datetime_fmt_ptr.is_null() {
            Some(ptr_to_str(datetime_fmt_ptr).map_err(|e| {
                PolarsError::ComputeError(format!("Invalid datetime format: {}", e).into())
            })?)
        } else {
            None
        };
