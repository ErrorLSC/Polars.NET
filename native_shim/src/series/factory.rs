...
use polars_arrow::array::{FixedSizeBinaryArray, FixedSizeListArray, Utf8ViewArray, View};
    ...
    ($func_name:ident, $rs_type:ty, $pl_type:ty) => {
        ...
        pub unsafe extern "C" fn $func_name(
            ...
            ffi_try!({
                ...
                // 3. Assemble
                let arrow_dtype = <$pl_type as PolarsDataType>::get_static_dtype()
                    .to_arrow(CompatLevel::newest());

                let arrow_array = PrimitiveArray::new(arrow_dtype, values_buffer, validity_bitmap);
...
pub unsafe extern "C" fn pl_series_new_bool(
    ...
    ffi_try!({
        ...
        let arrow_array = BooleanArray::new(ArrowDataType::Boolean, values_bitmap, validity_bitmap);
...
pub unsafe extern "C" fn pl_series_new_str_simd(
    ...
    ffi_try!({
        ...
        let array =
            Utf8ViewArray::try_new(ArrowDataType::Utf8View, views_buffer, buffers, validity)
                .expect("Failed to build Utf8ViewArray from C# buffers");

        // Convert to Series
        let series = Series::from_arrow(PlSmallStr::from_str(name_str.as_ref()), Box::new(array))
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn pl_series_new_guid_fixed_binary(
    name: *const c_char,
    values_ptr: *const u8,   // Flat contiguous GUID byte buffer (len * 16)
    validity_ptr: *const u8, // Arrow validity bitmap
    len: usize,              // Element count (number of Guids)
) -> *mut SeriesContext {
    ffi_try!({
        let name_str = unsafe { CStr::from_ptr(name).to_string_lossy() };

        // 1. Data Buffer: 16 bytes per GUID
        let total_bytes = len * 16;
        let data_buffer = if !values_ptr.is_null() && total_bytes > 0 {
            let data_slice = unsafe { std::slice::from_raw_parts(values_ptr, total_bytes) };
            Buffer::from(data_slice.to_vec())
        } else {
        ...
        let validity = if validity_ptr.is_null() {
            ...
            let v_slice = unsafe { std::slice::from_raw_parts(validity_ptr, bytes_len) };
            let v_vec = v_slice.to_vec();
            Some(Bitmap::try_new(v_vec, len).expect("Invalid validity bitmap for GUID series"))
        };

        // 3. Build Arrow FixedSizeBinaryArray (16 bytes)
        let array = FixedSizeBinaryArray::try_new(
            ArrowDataType::FixedSizeBinary(16),
            data_buffer,
            validity,
        )
        .expect("Failed to build FixedSizeBinaryArray from C# buffers");

        // 4. Convert directly to Polars Series (polars Binary type)
        let series = Series::from_arrow(PlSmallStr::from_str(name_str.as_ref()), Box::new(array))
            .expect("Failed to create Series from FixedSizeBinaryArray");
...
pub unsafe extern "C" fn pl_series_new_datetime(
    ...
    ffi_try!({
        ...
        let arrow_array = PrimitiveArray::new(ArrowDataType::Int64, vec.into(), validity_bitmap);
...
macro_rules! create_physical_ca {
    ($name:ident, $ptr:ident, $len:ident, $validity:ident, $phys_ty:ty, $arrow_ty:expr, $ca_ty:ident) => {{
        ...
        let arrow_array = PrimitiveArray::new($arrow_ty, vec.into(), validity_bitmap);
...
pub unsafe extern "C" fn pl_series_new_duration(
    ...
    ffi_try!({
        ...
        Ok(Box::into_raw(Box::new(SeriesContext {
            series: ca_duration.into_series(),
        })))
    })
}
...
pub unsafe extern "C" fn pl_series_new_decimal(
    ...
    ffi_try!({
        ...
        let data_type = ArrowDataType::Decimal(if precision == 0 { 38 } else { precision }, scale);

        let arrow_array = PrimitiveArray::new(data_type, vec.into(), validity_bitmap);
    ...
    ($func_name:ident, $rust_ty:ty, $arrow_ty:expr) => {
        ...
        pub unsafe extern "C" fn $func_name(
            ...
            ffi_try!({
                ...
                let inner_array = PrimitiveArray::new($arrow_ty, vec.into(), None);
                ...
                let inner_field =
                    polars_arrow::datatypes::Field::new("item".into(), $arrow_ty, false);
                let list_dtype = ArrowDataType::FixedSizeList(Box::new(inner_field), width);
...
#[unsafe(no_mangle)]
pub extern "C" fn pl_series_new_null(name: *const c_char, len: usize) -> *mut SeriesContext {
    ffi_try!({
        let name_str = unsafe { CStr::from_ptr(name).to_string_lossy() };
