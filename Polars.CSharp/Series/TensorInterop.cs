...
public partial class Series : IDisposable,IPolarsSeries
{
    ...
    /// </summary>
    /// <remarks>
    /// DANGER (LIFECYCLE WARNING): This is a zero-copy operation. The returned <see cref="nint"/> points 
    ...
    /// <returns>A tuple containing the raw <see cref="nint"/> to the first element, and a <c>long[]</c> representing the tensor shape.</returns>
    public (nint DataPointer, long[] Shape) AsDangerousUnmanagedTensor<T>() where T : unmanaged
    {
        if (!IsContiguous)
        {
            throw new InvalidOperationException(
    ...
    public (nint DataPointer, long[] Shape) AsDangerousUnmanagedTensor<T>(ReadOnlySpan<nint> shape) where T : unmanaged
    {
        if (!IsContiguous)
        {
            throw new InvalidOperationException(
