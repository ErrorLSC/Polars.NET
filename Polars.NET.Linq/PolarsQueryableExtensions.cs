...
public static class PolarsQueryableExtensions
{
    ...
    /// <summary>
    /// Translates and eagerly executes the LINQ query, materializing the scalar results directly into a Polars Series.
    /// Maintains zero-copy performance by keeping data in the unmanaged Rust/Arrow memory space.
    /// </summary>
    /// <typeparam name="T">The scalar type of the elements.</typeparam>
    /// <param name="query">The LINQ query.</param>
    /// <param name="seriesName">Optional: The name for the resulting Series. Defaults to empty/original.</param>
    /// <returns>A materialized Polars Series.</returns>
    public static IPolarsSeries ToISeries<T>(this IQueryable<T> query, string seriesName = "")
    {
        using var df = query.ToIDataFrame();

        var series = df.Column(0); 

        if (!string.IsNullOrEmpty(seriesName))
        {
            series.Rename(seriesName);
        }
    ...
    /// <summary>
    /// Asynchronously translates and eagerly executes the LINQ query into a Polars Series.
    /// </summary>
    public static async Task<IPolarsSeries> ToISeriesAsync<T>(
        this IQueryable<T> query, 
        string seriesName = "",
        CancellationToken cancellationToken = default)
    {
