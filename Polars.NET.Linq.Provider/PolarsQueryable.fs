...
type PolarsQueryProvider(initialLazyFrame: LazyFrameHandle, materializer: IDataFrameMaterializer) =
    ...
    member private this.ExecuteScalarInternal<'TResult>(expression: Expression) : 'TResult =
        ...
        | MethodCall(m, null, [ source ]) when m.Name = "Single" || m.Name = "SingleOrDefault" ->
            ...
            if not hasClientPreds then
                // Vectorized pushdown: slice top 2 rows to evaluate exact cardinality
        ...
        | MethodCall(m, null, [ source; itemExpr ]) when m.Name = "Contains" ->
            ...
            if not hasClientPreds && not (isNull itemVal) then
                ...
                // Case A: Primitive / String single-column DataFrame
                if itemType.IsPrimitive || itemType = typeof<string> || itemType = typeof<decimal> then
                    ...
                    // Case B: Composite object / record struct comparison (matching all properties)
            ...
            let rec tryGetSecondQueryPlan (e: Expression) : (LazyFrameHandle * bool) option =
                ...
                | :? MethodCallExpression as mc when typeof<IQueryable>.IsAssignableFrom(mc.Type) ->
                    ...
                    | :? IPolarsPlanSource as planSource ->
                        let lf = planSource.GetRawLazyFrameHandle()
                        let getPlanMethod = q.GetType().GetMethod("GetCompiledPlan", BindingFlags.Instance ||| BindingFlags.NonPublic ||| BindingFlags.Public)
                        let res = getPlanMethod.Invoke(q, null)
                        let tupleProps = res.GetType().GetProperties(BindingFlags.Public ||| BindingFlags.Instance)
                        let preds = tupleProps.[1].GetValue(res) :?> IEnumerable
                        Some (lf, preds.GetEnumerator().MoveNext())
                    | _ -> None
                | _ ->
                    match tryEvaluate e with
                    | Some (:? IPolarsPlanSource as planSource) ->
                        Some (planSource.GetRawLazyFrameHandle(), false)
                    | _ -> None
...
and PolarsQuery<'T> internal (lazyFrameHandle: LazyFrameHandle, materializer: IDataFrameMaterializer, exprOpt: Expression option) as this =
    ...
    static member internal ResolveToLazyFrameHandle(expr: Expression) : LazyFrameHandle option =
        ...
        // Safely extract the raw underlying object
        let rawObjOpt =
        ...
        // 1. Direct plan source (PolarsQuery or any wrapper implementing IPolarsPlanSource)
        | Some (:? IPolarsPlanSource as plan) ->
            Some (plan.GetRawLazyFrameHandle())

        // 2. Direct native handles
        ...
        // 3. High-level DataFrame or LazyFrame C# object via Handle property reflection
        | Some value ->
            ...
            if not (isNull handleProp) then
                ...
                // 4. In-memory sequence (List<T>, Array, IEnumerable<T>) -> Convert via DataFrameBuilder.FromRows<T>
    ...
    static member private TryExtractGroupAgg (groupParamName: string) (resLambda: LambdaExpression) : (string * ExprHandle) option =
        ...
        | :? NewExpression as newExpr ->
            ...
            |> List.tryPick (fun (memName, arg) ->
                match arg with
                // Matches empGroup.Count() or Enumerable.Count(empGroup)
    ...
    /// Universally extracts column renaming mappings from a two-parameter result selector: (left, right) => new T(...)
    ...
    member private this.CompilePipeline() : QueryOp list * (Func<'T, bool> list) =
        ...
        let rec fuse (remaining: LinqStage list) (opsAcc: QueryOp list) (clientPreds: Func<'T, bool> list) =
            ...
            // Rule: GroupJoin -> Native Pre-Aggregated Left Join Pushdown
            | LinqStage.GroupJoin(_, innerExpr, outerKey, innerKey, resSel) :: tail when clientPreds.IsEmpty ->
            ...
            // Rule: DefaultIfEmpty -> Passthrough to final materialization
    ...
    member private this.ExecuteQuery() : IEnumerable<'T> =
        ...
        // Case 3: Flat row materialization (standard entities, DTOs, value tuples)
            ...
            // Check if pipeline contains DefaultIfEmpty
